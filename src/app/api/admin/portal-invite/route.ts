...
export async function POST(req: NextRequest) {
  ...
  if (existing) {
    // User exists — resend invite by finding their auth user and sending another invite email
    const { data: { users: authUsers } } = await admin.auth.admin.listUsers({ page: 1, perPage: 1000 })
    const authUser = authUsers.find(u => u.email === existing.email)
    if (authUser) {
      await admin.auth.admin.inviteUserByEmail(existing.email)
    }
    authUserId = existing.id
  } else {
    // New invite — create auth user via invite (sends email with magic link)
