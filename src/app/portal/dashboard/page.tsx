...
export default function PortalDashboardPage() {
  ...
  // Amounts are shown in the customer's own currency (051).
  const currency = customer?.currency ?? 'XCG'
        ...
        <div className="flex items-center gap-2.5 bg-red-50 dark:bg-red-950/30 border border-red-200 dark:border-red-900 rounded-lg px-3 py-2">
          ...
          <div className="flex-1 min-w-0">
            <p className="font-semibold text-red-700 dark:text-red-400 text-sm">
              {openInvoices.length} open invoice{openInvoices.length > 1 ? 's' : ''} — {currency} {openInvoiceTotal.toFixed(2)}
            </p>
        ...
        <CardContent className="py-2 px-3">
          <div className="flex items-center justify-between gap-2">
            ...
            <div className="text-right shrink-0 flex items-center gap-1">
              <p className="font-bold text-red-600 text-sm">{(order as any).currency ?? 'XCG'} {Number(order.total).toFixed(2)}</p>
