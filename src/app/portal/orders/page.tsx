    ...
    <div className="space-y-3">
      <div className="flex items-center justify-between gap-3">
        <div>
          ...
          <p className="text-muted-foreground text-xs">{orders.length} total orders</p>
        </div>
        <Link href="/portal/new-order">
...
function OrderCard({ order, dimmed }: { order: Order; dimmed?: boolean }) {
  ...
  return (
    ...
    <Card className={`py-0 ${dimmed ? 'opacity-70' : ''} hover:border-red-300 transition-colors cursor-pointer`}>
      <CardContent className="py-2.5 px-3 space-y-2">
        <div className="flex items-start justify-between gap-2">
          ...
          <Badge className={`${status.color} flex items-center gap-1 text-[11px] px-1.5 py-0 shrink-0`}>
        ...
        {order.planned_date && (order.status === 'processing' || order.status === 'out_for_delivery') && (
          <div className="flex items-center gap-2 text-[11px] text-muted-foreground bg-muted/50 rounded-md px-2.5 py-1.5">
